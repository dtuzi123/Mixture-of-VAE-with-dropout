...
def autoencoder(x_hat, x, dim_img, dim_z, n_hidden, keep_prob, last_term):
    # encoding
    mu1, sigma1, mix1 = Create_CIFAR10_Encoder(x_hat,dim_z, "encoder1")
    mu2, sigma2, mix2 = Create_CIFAR10_Encoder(x_hat,dim_z, "encoder2")
    mu3, sigma3, mix3 = Create_CIFAR10_Encoder(x_hat,dim_z, "encoder3")
    mu4, sigma4, mix4 = Create_CIFAR10_Encoder(x_hat, dim_z,"encoder4")
    ...
    # decoding
    y1 = Create_CIFAR10_SubDecoder_(z1_samples, 64, "decoder1")
    y2 = Create_CIFAR10_SubDecoder_(z2_samples, 64, "decoder2")
    y3 = Create_CIFAR10_SubDecoder_(z3_samples, 64, "decoder3")
    y4 = Create_CIFAR10_SubDecoder_(z4_samples, 64, "decoder4")

    y1 = tf.reshape(y1,(-1,4* 512))
    y2 = tf.reshape(y2,(-1,4* 512))
    y3 = tf.reshape(y3,(-1,4* 512))
    y4 = tf.reshape(y4,(-1,4* 512))
    ...
    y1 = tf.reshape(y1,(batch_size,2,2,512))
    y2 = tf.reshape(y2,(batch_size,2,2,512))
    y3 = tf.reshape(y3,(batch_size,2,2,512))
    y4 = tf.reshape(y4,(batch_size,2,2,512))

    y = y1 + y2 + y3 + y4
    y = Create_CIFAR10_FinalDecoder_(y,64,"final")
...
with tf.Session() as sess:
    ...
    if isWeight:
        saver.restore(sess, 'models/Dropout_Simple_CIFAR4_Gaussian')
        import glob
    ...
    for epoch in range(n_epochs):
        ...
        if bestScore > loss_likelihood:
            ...
            saver.save(sess, "models/Dropout_Simple_CIFAR4_Gaussian")
