...
with tf.Session() as sess:
    ...
    if isWeight:
        saver.restore(sess, 'F:/Third_Experiment/models/Dropout_Simple_ImageNet_Bernoulli')
        import glob

        '''
        # load dataset
        img_path = glob.glob('C:/commonData/train_64x64 (2)/train_64x64/*.jpg')  # 获取新文件夹下所有图片
        data_files = img_path
    ...
    # load dataset
    img_path = glob.glob('C:/commonData/train_64x64/train_64x64/*.png')  # 获取新文件夹下所有图片
    data_files = img_path
    data_files = sorted(data_files)
    data_files = np.array(data_files)  # for tl.iterate.minibatches
    n_examples = 202599
    ...
    for epoch in range(n_epochs):
        ...
        if bestScore > loss_likelihood:
            ...
            saver.save(sess, "F:/Third_Experiment/models/Dropout_Simple_ImageNet_Bernoulli")

        if epoch > 0:
            x_fixed_image = np.reshape(x_fixed, (-1, 64, 64, 3))
            ims("results/" + "Real" + str(epoch) + ".jpg", merge2(x_fixed_image[:64], [8, 8]))

    # saver.save(sess, "F:/Third_Experiment/Dropout_Simple_")
